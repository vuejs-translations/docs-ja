...
マイナーおよびメジャーリリースは通常、**アルファ**、**ベータ**、そして**リリース候補 (RC)**という一連のプレリリース段階を経て提供されます。プレリリースの数と種類は変更の規模によって異なります。たとえば、更新が限定的なマイナーリリースはベータ段階のみの場合がありますが、メジャーリリースは通常、徹底的なテストとコミュニティのフィードバックのために 3 つの段階すべてが含まれます。

最新のプレリリースは npm から `npx install-vue@alpha`、`npx install-vue@beta`、または `npx install-vue@rc` を使ってインストールできます。タグ付きプレリリースにまだ含まれていない変更をテストするには、[vuejs/core](https://github.com/vuejs/core) リポジトリへのすべてのコミットが一時的な継続リリースプレビューとして公開されており、`npx install-vue@edge` を使ってインストールできます。
