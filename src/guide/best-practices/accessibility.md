...
再利用可能なコンポーネント内でこのパターンを使用する場合は、ID をハードコードする代わりに [`useId()`](/api/composition-api-helpers.html#useid) を使って生成してください。これにより、コンポーネントの各インスタンスの `id` の値が一意に保たれつつ、表示されているテキストがフォームコントロールに関連付けられます:
