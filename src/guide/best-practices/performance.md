...
時には、より良い抽象化やコード構成のために、[レンダーレスコンポーネント](/guide/components/slots.html#renderless-components)や高階コンポーネント（つまり、特別なプロパティで他のコンポーネントをレンダリングするコンポーネント）を作ることもあります。これは悪いことではありませんが、コンポーネントインスタンスはプレーンな DOM ノードよりもはるかに高価であり、抽象化パターンによりそれらを大量に生成すると、パフォーマンスコストが発生することを覚えておいてください。
