...
また、JavaScript のトランジションフックでは、コンポーネントの現在の状態に応じて異なる動作を適用することもできます。最後に、動的なトランジションを作成する究極の方法は、トランジションの性質を変更するプロパティを受け取る[再利用可能なトランジション](#reusable-transitions)を利用することです。安っぽく聞こえるかもしれませんが、制限はあなたの想像力だけです。
