...
IDE 上で分割代入された `props` と通常の変数をより視覚的に区別したい場合、Vue の VSCode 拡張機能には、分割代入された `props` にインラインヒントを表示する設定があります。
...
通常の `prop` を `watch(() => props.foo, ...)` と監視できるのと同様に、分割代入された `prop` もゲッターでラップすることで監視できます：

```js
watch(() => foo, /* ... */)
```

さらに、リアクティビティを保持しながら分割代入された `prop` を外部関数に渡す必要がある場合、これが推奨されるアプローチです：
