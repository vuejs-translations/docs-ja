...
IDE上で分割代入されたpropsと通常の変数をより視覚的に区別したい場合、VueのVSCode拡張機能には、分割代入されたpropsにインラインヒントを表示する設定があります。
...
通常のpropを `watch(() => props.foo, ...)` と監視できるのと同様に、分割代入されたpropもゲッターでラップすることで監視できます：
