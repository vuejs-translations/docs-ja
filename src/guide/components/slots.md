...
::: tip 備考
スロットコンテンツは親のレンダースコープの一部であるため、子コンポーネントで宣言された `<style scoped>` ブロックは、スロットコンテンツには適用され**ません**。スロットされたコンテンツにスタイルを適用する必要がある場合は、親コンポーネント側で行うか、子のスコープ付きスタイルシートで `:deep()` 修飾子を使用してください（詳細は [スコープ付き CSS](/api/sfc-css-features#scoped-css) を参照）。
:::
...
[Playground で試す](https://play.vuejs.org/#eNplj8EKgzAQRH9l2UsvrdKrWKH4A/2AXIpuMRCzIVmlIP57EwMF9TgzyczbBZ/OFfNEWGEdOq+dQCCZXKOsHh17gXbQpm85CktW4ON5hEtR7u1UcVG2LnNH/B2F0OjMWygqgPrQM9+CYXko9NSRnql/eXZB4fYYYFlgFxRCX4F1PQcdTzYl28gBq0lIfwy84pk6Hb4HTVwZIm1GwoGMYYXZq7a96N6zUx421h9AQHet)
