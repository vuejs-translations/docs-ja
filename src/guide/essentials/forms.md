...
HTML 組み込みの input タイプが常にあなたのニーズを満たすとは限りません。幸いなことに、 Vue のコンポーネントでは完全にカスタマイズされた動作を持つ再利用可能な入力を構築することが可能です。それらの入力は `v-model` でも動作します！　詳しくは、コンポーネントガイドの [`v-model` の使い方](/guide/components/v-model.html) を参照してください。
