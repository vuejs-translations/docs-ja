...
`<script setup>` を使用しない場合は、以下のように `setup()` で参照を一緒に返す必要があります:
...
watchEffect(() => {
  if (input.value) {
    ...
    // 要素がまだマウントされていない、またはアンマウントされた (v-if など)
  }
})
```
...
特例は、`<script setup>` を使用するコンポーネントです。このようなコンポーネントは、デフォルトで**プライベート**となります。`<script setup>` が使われている子コンポーネントを親コンポーネントから参照する場合、親コンポーネントは、子コンポーネントで `defineExpose` マクロを使って選択されたパブリックインターフェース以外のものにアクセスすることができません:
...
親がテンプレート参照を用いてこのコンポーネントのインスタンスを取得する場合、取得されるインスタンスは `{ a: number, b: number }` の形になります (通常のインスタンスと同様に、参照は自動的にアンラップされます)。
