...
`track()` の内部では、現在実行中のエフェクトをがあるかどうかをチェックします。ある場合は、追跡しているプロパティの購読者であるエフェクトを（Set に格納）を検索し、そのエフェクトをを Set に追加します:

```js
// これは、エフェクトをの実行直前に設定されます。
// これについては後ほど扱います。
let activeEffect
...
この時点で、依存関係を自動的に追跡し、依存関係が変更されるたびに再実行するエフェクトが作成しました。これを **リアクティブエフェクト（Reactive Effect）** と呼びます。
