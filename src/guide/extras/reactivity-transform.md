...
2. [型のみの props 宣言](/api/sfc-script-setup#type-only-props-emit-declarations)を使う場合、props のデフォルト値を宣言するための簡単な方法はありません。この目的のために `withDefaults()` を導入しましたが、まだ使い勝手が悪いです。
