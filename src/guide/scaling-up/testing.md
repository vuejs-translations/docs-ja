...
- [Jest](https://jestjs.io/) はよく知られた単体テストフレームワークで、[vite-jest](https://github.com/sodatea/vite-jest) パッケージによって Vite で動作させることができます。ですが、Jest を推奨するのは既存の Jest テストスイートを Vite ベースのプロジェクトに移行する必要がある場合のみです。Vitest はよりシームレスな統合と優れたパフォーマンスを提供するからです。
...
コンポーネントのテストでは、内部実装の詳細よりもそのコンポーネントのパブリックなインターフェイスに注目する必要があります。ほとんどのコンポーネントでは、パブリックインターフェースは発行されるイベント、プロパティ、スロットに限定されています。テストする際は**コンポーネントが何をするのかをテストするのであって、どのようにするのかをテストするのではない**ことを忘れないようにしましょう。
...
- **ふるまい**のロジック: ユーザーの入力イベントに対応した更新内容のレンダリングやイベント発行を評価します。
  ...
  以下の例では、"increment" というラベルの付いた DOM 要素を持ち、クリックできるステッパー（Stepper: 値が増えていく）コンポーネントを示しています。`max` というプロパティを渡すことで、ステッパーが `2` を超えてインクリメントすることを防ぎます。したがって、ボタンを 3 回クリックしても、UI は `2` と表示されるはずです。

  私たちはステッパーの実装について何も知りません。ただ、「入力」が `max` プロパティで、「出力」がユーザーに表示される DOM の状態であることだけが分かっています。
