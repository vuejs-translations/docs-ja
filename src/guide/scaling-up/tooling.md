...
## バックエンドフレームワークとの統合 {#backend-framework-integrations}

[Laravel](https://laravel.com/) で Vue を使用している場合、このフレームワークにはアセットのバンドルとホットモジュールリプレースメントをすぐに扱える公式の [Vite プラグイン](https://laravel.com/docs/vite)が同梱されています。

その他のバックエンドについては、Vite の[バックエンド統合ガイド](https://vite.dev/guide/backend-integration.html)を参照して、手動で連携させてください。
