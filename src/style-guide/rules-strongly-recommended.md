...
**コンテンツを持たないコンポーネントは、[単一ファイルコンポーネント](/guide/scaling-up/sfc)、文字列テンプレート、および [JSX](/guide/extras/render-function.html#jsx-tsx) では自己閉鎖されるべきですが、DOMテンプレート内ではそうしてはいけません。**
