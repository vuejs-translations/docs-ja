...
**[単一ファイルコンポーネント](/guide/scaling-up/sfc) のファイル名は、すべてパスカルケース（PascalCase）にするか、すべてケバブケース（kebab-case）にするべきです。**
...
**コンテンツを持たないコンポーネントは、[単一ファイルコンポーネント](/guide/scaling-up/sfc)、文字列テンプレート、および [JSX](/guide/extras/render-function#jsx-tsx) では自己閉鎖されるべきですが、DOM 内テンプレート内ではそうしてはいけません。**
...
**ほとんどのプロジェクトにおいて、[単一ファイルコンポーネント](/guide/scaling-up/sfc)と文字列テンプレートの中では、コンポーネント名は常にパスカルケース（PascalCase）になるべきです。しかし、 DOM 内テンプレートの中ではケバブケース（kebab-case）です。**
...
**JS/[JSX](/guide/extras/render-function#jsx-tsx)内でのコンポーネント名は常にパスカルケース（PascalCase）にするべきです。ただし、`app.component` で登録したグローバルコンポーネントしか使わないような単純なアプリケーションでは、ケバブケース（kebab-case）を含む文字列になるかもしれません。**
...
## props 名の型式 {#prop-name-casing}

**props 名は、定義の時は常にキャメルケース（camelCase）を使用する必要があります。DOM 内テンプレート内で使用する場合、props はケバブケース（kebab-case）を使用する必要があります。単一ファイルコンポーネントのテンプレートと [JSX](/guide/extras/render-function#jsx-tsx) では、ケバブケースまたはキャメルケースのどちらかの props を使用できます。ケーシングは一貫している必要があります - キャメルケースの props を使用することにした場合、そのアプリケーション内ではケバブケースを使用しないでください**
...
JavaScript では、複数のプロパティをもつ要素を複数の行に分けて書くことはよい慣習だと広く考えられています。なぜなら、その方がより読みやすいからです。Vue のテンプレートや [JSX](/guide/extras/render-function#jsx-tsx)  も同じように考えることがふさわしいです。
